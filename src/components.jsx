    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 p-6 text-center font-sans">
          <div className="bg-white dark:bg-ios-card p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-ios-border max-w-sm w-full">
...
export function LogItem({ log, onDelete, onEdit, simple = false }) {
  ...
  return (
    <div className={`group bg-white dark:bg-ios-card rounded-[1.5rem] border transition-all flex justify-between items-start ${simple ? 'p-3' : 'p-5'} ${isProgression ? 'border-orange-300 bg-orange-50/50 dark:bg-orange-900/20 dark:border-orange-700' : 'border-slate-100 dark:border-ios-border hover:shadow-md'}`}>
      <div className="flex gap-4 items-start">
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {isSymptom && <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${log.severity > 7 ? 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-gray-400'}`}>{log.severity}级</span>}
            {!isSymptom && <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-gray-400 font-medium">{log.method === 'other' ? log.methodLabel : MEDICATION_METHODS.find(m => m.id === log.method)?.label}</span>}
          </div>
          <p className="text-sm text-slate-500 dark:text-gray-400 leading-snug break-all">{isSymptom ? (log.note || '') : `${log.dosage} ${log.reason ? `• ${log.reason}` : ''}`}</p>
