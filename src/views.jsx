...
export function CourseDetailView({ course, logs, onUpdateStatus, onDeleteLog, onEditLog }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="bg-white dark:bg-ios-card p-6 rounded-[2rem] border border-slate-100 dark:border-ios-elevated shadow-sm relative overflow-hidden">
        ...
        <div className="bg-slate-50 dark:bg-ios-elevated rounded-2xl p-5 space-y-4 border border-slate-100 dark:border-ios-border">
           ...
           {course.hasDoctorVisit && (
             <div className="flex items-start gap-3 pt-4 border-t border-slate-200 dark:border-ios-border"><Stethoscope className="w-4 h-4 text-blue-500 mt-1 shrink-0" /><div className="flex-1"><span className="text-xs text-slate-400 block mb-1">就诊记录 ({course.department} {course.visitDate})</span><p className="text-sm font-semibold text-slate-700 dark:text-slate-200">诊断：{course.diagnosis || '未填写'}</p>{course.prescription && <div className="mt-3 bg-white dark:bg-ios-card p-3 rounded-xl border border-slate-200 dark:border-ios-border text-xs text-slate-600 dark:text-slate-400 leading-relaxed"><span className="font-bold block mb-1 text-slate-700 dark:text-slate-300">处方/医嘱：</span>{course.prescription}</div>}</div></div>
           )}
        </div>
        <div className="grid grid-cols-2 gap-4 text-sm mt-4">
           <div className="bg-slate-50 dark:bg-ios-elevated p-4 rounded-2xl"><span className="text-slate-400 text-xs block mb-1">开始日期</span><span className="font-mono text-slate-700 dark:text-slate-200">{course.startDate}</span></div>
           <div className="bg-slate-50 dark:bg-ios-elevated p-4 rounded-2xl"><span className="text-slate-400 text-xs block mb-1">持续天数</span><span className="font-mono text-slate-700 dark:text-slate-200">{isRecovered && course.endDate ? Math.ceil((safeDate(course.endDate) - safeDate(course.startDate)) / (1000 * 60 * 60 * 24)) + 1 : getDaysSince(course.startDate)} 天</span></div>
        </div>
      </div>
      <div>
        <h3 className="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-4 px-2">病程时间轴</h3>
        <div className="space-y-8 relative pl-4 border-l-2 border-slate-100 dark:border-ios-elevated ml-4">
  ...
  return (
    <div className="space-y-4 animate-fade-in">
       <div className="sticky top-0 bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-sm pt-2 pb-4 z-10">
         <div className="relative"><Search className="absolute left-4 top-4 w-5 h-5 text-slate-400" /><input ref={inputRef} type="text" placeholder="搜索症状、药品、病程、日期..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-3.5 bg-white dark:bg-ios-card border border-slate-200 dark:border-ios-elevated rounded-2xl shadow-sm outline-none focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900 focus:border-indigo-300 dark:focus:border-indigo-700 transition-all text-ios-elevated dark:text-slate-100" />{searchTerm && <button onClick={() => setSearchTerm('')} className="absolute right-4 top-4 text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button>}</div>
       </div>
...
export function SettingsView({ onExport, onImport, fileInputRef, handleImport, webdavConfig, setWebdavConfig, onSync }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-white dark:bg-ios-card rounded-[2rem] border border-slate-100 dark:border-ios-elevated shadow-sm overflow-hidden p-5">
        ...
        {showWebDav && <div className="mt-4 space-y-4 bg-slate-50 dark:bg-ios-elevated p-5 rounded-2xl text-sm animate-fade-in"><div className="p-3 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 rounded-xl border border-amber-200 dark:border-amber-800 text-xs leading-relaxed">⚠️ 注意：浏览器由于安全策略(CORS)，可能无法直接连接网盘。</div><div><label className="block text-slate-500 dark:text-slate-400 mb-1.5 text-xs font-bold uppercase">服务器 URL</label><input type="text" value={webdavConfig.url} onChange={(e) => setWebdavConfig({...webdavConfig, url: e.target.value})} className="w-full p-3 border border-slate-200 dark:border-ios-border bg-white dark:bg-ios-card rounded-xl outline-none focus:border-indigo-400 transition-colors text-ios-elevated dark:text-slate-100" /></div><div><label className="block text-slate-500 dark:text-slate-400 mb-1.5 text-xs font-bold uppercase">账号</label><input type="text" value={webdavConfig.username} onChange={(e) => setWebdavConfig({...webdavConfig, username: e.target.value})} className="w-full p-3 border border-slate-200 dark:border-ios-border bg-white dark:bg-ios-card rounded-xl outline-none focus:border-indigo-400 transition-colors text-ios-elevated dark:text-slate-100" /></div><div><label className="block text-slate-500 dark:text-slate-400 mb-1.5 text-xs font-bold uppercase">密码</label><input type="password" value={webdavConfig.password} onChange={(e) => setWebdavConfig({...webdavConfig, password: e.target.value})} className="w-full p-3 border border-slate-200 dark:border-ios-border bg-white dark:bg-ios-card rounded-xl outline-none focus:border-indigo-400 transition-colors text-ios-elevated dark:text-slate-100" /></div><button onClick={onSync} className="w-full py-3 bg-blue-600 text-white rounded-xl flex items-center justify-center gap-2 hover:bg-blue-700 transition-colors font-bold"><RefreshCw className="w-4 h-4" /> 立即同步上传</button></div>}
      </div>
      <div className="bg-white dark:bg-ios-card rounded-[2rem] border border-slate-100 dark:border-ios-elevated shadow-sm overflow-hidden"><button onClick={onExport} className="w-full flex items-center justify-between p-5 hover:bg-slate-50 dark:hover:bg-ios-elevated border-b border-slate-100 dark:border-ios-elevated transition-colors"><span className="flex items-center gap-3 font-bold text-sm text-slate-700 dark:text-slate-200"><Download className="w-5 h-5 text-emerald-500"/> 导出备份 (JSON)</span><ChevronRight className="w-4 h-4 text-slate-300" /></button><button onClick={onImport} className="w-full flex items-center justify-between p-5 hover:bg-slate-50 dark:hover:bg-ios-elevated transition-colors"><span className="flex items-center gap-3 font-bold text-sm text-slate-700 dark:text-slate-200"><Upload className="w-5 h-5 text-amber-500"/> 恢复数据</span><ChevronRight className="w-4 h-4 text-slate-300" /></button><input type="file" ref={fileInputRef} onChange={handleImport} className="hidden" accept=".json"/></div>
    </div>
  );
}
...
export function StatsView({ logs }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="bg-white dark:bg-ios-card rounded-[2rem] border border-slate-100 dark:border-ios-elevated shadow-sm p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2"><button onClick={() => changeMonth(-1)} className="p-2 hover:bg-slate-100 dark:hover:bg-ios-elevated rounded-full transition-colors"><ChevronLeft className="w-5 h-5 text-slate-600 dark:text-slate-400"/></button><button onClick={() => changeMonth(1)} className="p-2 hover:bg-slate-100 dark:hover:bg-ios-elevated rounded-full transition-colors"><ChevronRight className="w-5 h-5 text-slate-600 dark:text-slate-400"/></button></div>
        </div>
        ...
        <div className="grid grid-cols-7 gap-2">
          ...
          {calendarData.days.map(day => {
             ...
             let bgClass = "bg-slate-50 dark:bg-ios-elevated";
      ...
      <div className="bg-white dark:bg-ios-card rounded-[2rem] border border-slate-100 dark:border-ios-elevated shadow-sm p-6">
        ...
        <div className="space-y-4">{partStats.length === 0 ? <p className="text-sm text-slate-400">暂无数据</p> : partStats.map(([part, count], index) => (<div key={part} className="flex items-center gap-4"><span className="text-xs font-mono text-slate-400 w-4">{index+1}</span><div className="flex-1"><div className="flex justify-between text-sm mb-2"><span className="font-medium text-slate-700 dark:text-slate-300">{part}</span><span className="text-slate-500 dark:text-slate-400 font-medium">{count}次</span></div><div className="w-full bg-slate-100 dark:bg-ios-elevated h-2.5 rounded-full overflow-hidden"><div className="bg-rose-400 h-full rounded-full" style={{ width: `${Math.min(100, (count / partStats[0][1]) * 100)}%` }}></div></div></div></div>))}</div>
      </div>
    </div>
  );
}
