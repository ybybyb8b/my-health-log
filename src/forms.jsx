...
export function NewCourseForm({ onSubmit }) {
  ...
  return (
    <div className="space-y-5 pb-10">
      <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">病程名称</label><input autoFocus type="text" value={data.name} onChange={(e) => setData({...data, name: e.target.value})} placeholder="例如：2024冬季甲流、急性肠胃炎..." className="w-full p-4 bg-slate-50 dark:bg-ios-elevated border-0 rounded-2xl focus:ring-2 focus:ring-indigo-100 dark:focus:ring-gray-700 outline-none transition-all text-slate-800 dark:text-white" /></div>
      <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">开始日期</label><input type="date" value={data.startDate} onChange={(e) => setData({...data, startDate: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-ios-elevated border-0 rounded-2xl focus:ring-2 focus:ring-indigo-100 dark:focus:ring-gray-700 outline-none transition-all text-slate-800 dark:text-white" /></div>
      <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">主要症状描述</label><textarea value={data.symptoms} onChange={(e) => setData({...data, symptoms: e.target.value})} placeholder="发烧、咳嗽、全身酸痛..." className="w-full p-4 bg-slate-50 dark:bg-ios-elevated border-0 rounded-2xl focus:ring-2 focus:ring-indigo-100 dark:focus:ring-gray-700 outline-none h-24 text-sm resize-none transition-all text-slate-800 dark:text-white" /></div>
...
export function SymptomForm({ onSubmit, defaultParts, customParts, onAddPart, activeCourses, editingLog }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">详细描述</label><textarea value={formData.note} onChange={(e) => setFormData({...formData, note: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-ios-elevated border-0 rounded-2xl focus:ring-2 focus:ring-indigo-100 dark:focus:ring-gray-700 outline-none text-sm min-h-[100px] resize-none transition-all text-slate-800 dark:text-white" placeholder={formData.isProgression ? "请详细描述病情发生了什么变化..." : "例如：刺痛、持续时间..."} /></div>
      <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">记录日期</label><input type="date" value={formData.recordDate} onChange={(e) => setFormData({...formData, recordDate: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-ios-elevated border-0 rounded-2xl focus:ring-2 focus:ring-indigo-100 dark:focus:ring-gray-700 outline-none text-sm text-slate-800 dark:text-white" /></div>
      <button onClick={() => { 
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-ios-elevated border-0 rounded-2xl focus:ring-2 focus:ring-indigo-100 dark:focus:ring-gray-700 outline-none transition-all text-slate-800 dark:text-white" placeholder="例如：奥司他韦..." />
        {/* 智能联想标签 */}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">用量</label><input type="text" value={formData.dosage} onChange={(e) => setFormData({...formData, dosage: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-ios-elevated border-0 rounded-2xl focus:ring-2 focus:ring-indigo-100 dark:focus:ring-gray-700 outline-none text-sm text-slate-800 dark:text-white" placeholder="如：75mg" /></div>
        <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">原因</label><input type="text" value={formData.reason} onChange={(e) => setFormData({...formData, reason: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-ios-elevated border-0 rounded-2xl focus:ring-2 focus:ring-indigo-100 dark:focus:ring-gray-700 outline-none text-sm text-slate-800 dark:text-white" placeholder="如：发热" /></div>
      </div>
